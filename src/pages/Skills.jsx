      ...
      <div className="relative z-20 max-w-7xl mx-auto px-6 w-full">
        ...
        <div className="space-y-24 md:space-y-32">
          {skillCategories.map((category) => {
            // Determine hover class based on category title
            let hoverEffect = "hover:-translate-y-2"; // default (Frontend)
            if (category.title.includes("Backend")) {
              hoverEffect = "hover:translate-y-2";
            } else if (category.title.includes("Languages")) {
              ...
              <div key={category.title}>
                ...
                <div className="flex flex-wrap gap-6 justify-center max-w-4xl mx-auto">
                  {category.skills.map((skill) => (
                    <div
                      key={skill.name}
                      className={`group flex items-center gap-4 px-6 py-4 rounded-2xl bg-white dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800 hover:border-orange-500/50 shadow-sm hover:shadow-xl hover:shadow-orange-500/10 transition-all duration-300 ${hoverEffect}`}
                    >
                      <img
